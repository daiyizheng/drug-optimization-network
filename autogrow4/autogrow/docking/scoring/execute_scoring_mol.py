...
def run_scoring_common(vars, smile_file, folder_to_search):
    ...
    # the ligand id full name 制作所有可能已对接的配体的字典。这将从 .smi 文件 gen_n_to_convert.smi 文件中获取。 键是配体 id 的缩写名称，项目是 SMILES 字符串和配体 id 全名
    ...
    # scoring function these may be different file types 查找所有需要评分的文件。因为这取决于评分函数，所以这些文件类型可能不同
    ...
    # 确定评分函数的值是否应根据配体中非H原子的数量进行调整，即resoring_lig_efficiency
...
def rescore_single_file(file_path, scoring_object):
    ...
    doesn't break.
      通过此函数运行scoring_object.run_rescoring，这样多线程就不会中断。
    Inputs:
...
def make_lig_score_dictionary(list_of_list_of_lig_data):
    ...
    for lig in list_of_list_of_lig_data:
        ...
        # Handle if Multiple Conformers and Poses 处理多构像和姿势
...
def score_files_multithread(scoring_object, file_path, rescore_lig_efficiency,
    ...
    if rescore_lig_efficiency is True: # 如果为True，则最终分数将调整为配体效率分数，否则将保留scoring_object的输出
...
def make_dict_of_smiles(smile_file):
    ...
    这将采用 .smi 文件并创建一个包含有关微笑的所有信息的字典。该列表还没有分数，但将包含所有字符串信息。这将在稍后搜索时使用。
