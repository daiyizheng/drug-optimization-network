...
class VinaDocking(ParentDocking):
    ...
    def rank_and_save_output_smi(self, vars, current_generation_dir,
        ...
        file.
        给定一个包含 PDBQT 的文件夹，根据对接分数（从高到低）对所有 SMILES 进行排名。然后将其格式化为 .smi 文件。然后保存文件。
        Inputs:
        ...
        # Get directory string of PDB files for Ligands 获取配体 PDB 文件的目录字符串
        folder_with_pdbqts = current_generation_dir + "PDBs" + os.sep

        # Run any compatible Scoring Function 运行任何兼容的评分功能
        smiles_list = Scoring.run_scoring_common(vars, smile_file, folder_with_pdbqts)

        # Before ranking these we need to handle Pass-Through ligands from the 在对这些进行排名之前，我们需要处理上一代的传递配体
        # last generation If it's current_gen_int==1 or if 如果 current_gen_int==1 或 vars['redock_elite_from_previous_gen'] 为 True
        # vars['redock_elite_from_previous_gen'] is True -Both of these states  这两种状态都对接了上一代的所有配体，因此所有传递配体都已经在 PDB 的文件夹中，因此它们应该在 smiles_list 中说明
        ...
        # for in smiles_list If vars['redock_elite_from_previous_gen'] is False 如果 vars['redock_elite_from_previous_gen'] 为 False 并且 current_gen_int != 1
        # and current_gen_int != 1 - We need to append the scores form the 我们需要将最后一代的分数附加到 smiles_list
        # last gen to smiles_list

        # Only add these when we haven't already redocked the ligand 仅当我们尚未重新对接配体时才添加这些
        if (
        ...
        # unique a molecule is from the other mols in the same generation 与组中其他配体相比，对每个配体的多样性进行评分，这会在最后一列中添加一个浮点数，以求成对比较的总和，多样性得分越低，分子与同一代中其他分子的差异越大
