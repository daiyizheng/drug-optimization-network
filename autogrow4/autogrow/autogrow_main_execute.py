...
def main_execute(vars):
    ...
    # Determine what was the last completed generation in the Run directory 确定Run目录中最后一次完成的生成是什么
    last_generation = determine_current_gen(output_directory)
    if last_generation is None:
        # Check to see if there's a Run 0 based on the seed. 检查是否有基于种子的运行0。
        if vars["use_docked_source_compounds"] is True:
            # This will assess and rank the source compounds prior to
            # generation 1. Thus using the source compounds as a generation 0 这将在第1代之前对源化合物进行评估和排序。因此使用源化合物作为第0代。
    ...
    # is broken into 3 main sections: 这是将控制和执行所有命令的主循环。它分为3个主要部分：
    ...
    #     PDB 1）用配体填充新一代的操作，所述配体既通过用户定义的过滤器又从1D smiles转换为3D PDB
    # 2)  Docking which handles converting from PDBs to Docking specific
    #     formats and running the actual Docking simulations 2）对接，用于处理从PDB转换为特定对接格式并运行实际对接模拟
    # 3)  Ranking the generation based on the Docking scores 3）根据停靠分数对生成进行排名
    for current_generation_number in range(starting_generation_num, num_gens_to_make+1):
        ...
        if current_generation_number == 0 and vars["use_docked_source_compounds"] is True:
            ...
            if already_docked is False:
                ...
                # name where the unweighted ranked but score .smi file resides 运行PDB到对接特定文件类型的文件转换，并开始对接unweighted_ranked_smile_file是未加权排名但评分的.smi文件所在的文件名
            ...
            # where the unweighted ranked but score .smi file resides 运行PDB到停靠特定文件类型的文件转换，并且Begin docking unweighted_ranked_smile_file是未加权排名但得分.smi文件所在的文件名
        ...
        # Delete all temporary files; Skip if in Debugging Mode 删除所有临时文件；如果处于调试模式则跳过
        if vars["debug_mode"] is False:
            ...
            # Delete Folders in an ordered manor incase folders are nested 删除有序庄园中的文件夹，以防文件夹嵌套
...
def find_last_generation(folder_path_string_no_gen):
    ...
    path exists as a directory. 这将采用一个结尾缺少整数的文件夹路径，并查找是否有任何文件夹与带有整数的路径一起存在。如果有，它将返回添加到路径时作为目录存在的最高整数。
    ...
    Starting fresh. 如果不存在路径为+0的目录，则返回None。这导致运行此尝试的起始生成为`generation_0`。重新开始。

    folder_path_string_no_gen = output_directory + "generation_" 
    folder_path_string_no_gen=输出目录+"生成"
    Inputs:
