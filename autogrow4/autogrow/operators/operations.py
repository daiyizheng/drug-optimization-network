...
def populate_generation(vars, generation_num):
    ...
    if generation_num == 1:
        ...
        # directly This will be done later but we are unpacking vars here 有多少直接从上一代前进到下一代这将在稍后完成，但我们在这里解压vars
    ...
    # Get the Source compound list. This list is the full population from 获取源化合物列表。
    # either the previous generations or if its Generation 1 than the its the 此列表是来自前几代的全部群体，或者如果其第一代比其整个用户指定的源化合物列表中的全部人口，
    # entire User specified Source compound list If either has a SMILES that 如果其中任何一个具有未在 RDKit 中清理的 SMILES，则它将被排除，并打印其名称和 SMILES 字符串将被打印。
    ...
    while len(new_mutation_smiles_list) < num_mutations:
        ...
        # Make all mutants 开始突变模块
        new_mutants = Mutation.make_mutants(
            vars, # 
            generation_num, # 第几代
            number_of_processors, # 进程
            num_mutants_to_make, # 需要突变的
            seed_list_mutations,# 起始化合物集合
            new_mutation_smiles_list, # 新的突变集合
            rxn_library_variables,
        )
    ...
    # Get unaltered samples from the previous generation 获取上一代未改变的样本
    ...
    # to next generation final selection通过精英主义列出选择传递到下一代的配体列表这处理创建种子列表并定义下一代最终选择的进展
...
def populate_generation_zero(vars, generation_num=0):
    ...
    the generation 0 这将处理0代重新启动所需的所有内容，并处理0代
    ...
    num_crossovers = 0
    num_mutations = 0
    # 获取源复合列表。此列表是前几代的完整填充，或者如果它的第1代是它的整个用户指定的源复合列表。
    ...
    # does not sanitize in RDKit it will be excluded and a printout of its
    # Name and SMILES string will be printed. 如果其中一个在RDKit中有未净化的SMILES，它将被排除在外，并打印其名称和SMILES字符串的打印输出。
    source_compounds_list = get_complete_list_prev_gen_or_source_compounds(
        vars, generation_num
    )# 预处理
    ...
    # generation final selection 列出所选配体以传递给下一代。这将处理创建种子列表并定义下一代最终选择的进展
    chosen_mol_to_pass_through_list = make_pass_through_list(
    ...
    # make a list of all the ligands from mutations, crossovers, and from the
    # last generation 列出所有来自突变、交叉和上一代的配体
    new_generation_smiles_list = []
    full_generation_smiles_list = []

    # These will be docked and scored for generation 0 这些将对接和评分为第0代
    for i in chosen_mol_to_pass_through_list:
        new_generation_smiles_list.append(i)
        full_generation_smiles_list.append(i)
    ...
    # Save the Full Generation # 保存完整一代分子
    full_generation_smiles_file, new_gen_folder_path = save_generation_smi(
        vars["output_directory"], generation_num, full_generation_smiles_list, None
    )

    # Save the File to convert to 3d 保存文件转换为3d
    smiles_to_convert_file, new_gen_folder_path = save_generation_smi(
        vars["output_directory"],
        generation_num,
        new_generation_smiles_list,
    ...
    # to do the following: 如果您要重新对接并转换零代，您还需要执行以下操作
    # 使用GYPSUM将SMILES转换为.sdf，并使用rdkit将.sdf转换为.pdb这将把sdf文件输出到文件夹中。
    ...
    # saved as .smi.2.sdf .smi.0.sdf文件不是有效的mol，但所有其他文件都是有效的。原始.smi文件中的第一个Smiles保存为.smi.1.sdf，第二个文件保存为.smi.2.sdf
...
def test_source_smiles_convert(smile_info):
    ...
    object 这将尝试将SMILES字符串转换为rdkit.Chem.rdchem.Mol对象
        -done in a try statement so that it is some bad SMILES string which is
        incapable of being converted 在try语句中完成，因此它是一些无法转换的坏SMILES字符串
        - it also checks that the SMILES string is able to be sanitized 它还检查SMILES字符串是否能够被清除
    ...
    # Check there are no * which are atoms with atomic number=0 检查是否有原子序数为0的原子*
    mol = MOH.check_for_unassigned_atom(mol) # 分子原子序号是从1开始的
    if mol is None:
...
def get_complete_list_prev_gen_or_source_compounds(vars, generation_num):
    ...
    compound list 从上一代源化合物列表中获取源化合物列表
    这也过滤了列表，以确保摩尔可以导入RDKit，并通过药物喜爱度过滤器。
    ...
    file 
    如果generation=1，则使用用户指定的起始化合物如果generation>1，则使用前几代的顶级配体。这需要一个.smi文件
    Inputs:
    :param dict vars: a dictionary of all user variables
    ...
    if generation_num == 0: # 这将是作为种子的起始分子的完整列表
        # This will be the full length list of starting molecules as the seed 
        source_file = str(vars["source_compound_file"]) # 分子源文件
    ...
    # 如果generation=1，则使用用户指定的起始化合物，并且0代源文件不存在
    ...
    else: # 如果generation>1，则使用前几代的顶级配体。
        source_file = vars[
            "output_directory"
        ...
        usable_list_of_smiles = Ranking.get_usable_format(source_file) # 拿到上一代打分的分子
        ...
        if len(usable_list_of_smiles) == 0:
            ...
            print(printout)
            raise Exception(printout)
    # 测试usable_list_of_smiles中的每个SMILES是否都是将在RDKit中导入并净化的有效SMILES。
    ...
    # sanitize 如果SMILES是碎片状的、包含没有原子序数（*）的原子或不能净化，则将被排除在外
    ...
    if vars["filter_source_compounds"] is True:# 过滤源分子文件

        prefilter_list = copy.deepcopy(usable_list_of_smiles)
        print("")
        print("Running Filter on the Compounds from last generation/Source")
        usable_list_of_smiles = Filter.run_filter(vars, usable_list_of_smiles)#过滤器

        # Remove Nones:移除None的分子
        usable_list_of_smiles = [x for x in usable_list_of_smiles if x is not None]
    ...
    random.shuffle(usable_list_of_smiles)# 随机排列
...
def make_seed_list(vars, source_compounds_list, generation_num, num_seed_diversity,
    ...
    file
    如果generation = 0，则使用用户指定的起始化合物。如果generation >0，则使用前几代顶级配体。这需要一个.smi 文件 
    Inputs:
    :param dict vars: a dictionary of all user variables
    ...
    if generation_num == 0:
        ...
        if vars["use_docked_source_compounds"] is False:
            ...
            if os.path.exists(source_file_gen_0) is False:
            ...
            else: # 如果源化合物小于第 1 代种子所需的种子池，则 Generation_num 1 可能会遇到问题。
                # generation_num 1 may run into problems if the source 因为种子选项与代数相关（由于多样性选项的贬值），因此我们可能需要忽略第 1 代种子的排名以适应较小的源大小。
                ...
                # but will provide a warning message. 这对于领先优化尤其重要，其中源池可能要小得多。因此，如果种子数量大于现有数量，我们将覆盖第 1 代的种子，但会提供警告消息。
                if (
    ...
    if full_length is True or generation_num == 0:
        # This will be the full length list of starting molecules as the seed 这将是作为种子的起始分子的完整长度列表
        ...
        # scores 根据多样性分数和对接分数获取 source_file 的子集
...
def determine_seed_population_sizes(vars, generation_num):
    ...
    该函数决定了有多少分子将因其对接分数而被选择来播种一代，以及有多少分子将因其多样性分数而被评估。
    Inputs:
    :param dict vars: a dictionary of all user variables
    :param int generation_num: the interger of the current generation
    ...
    """
    # How many fewer seed mols are chosen from diversity compared to the 1st generation This is also how many more get chosen from elitist selection
    #  # 与第一代相比，从多样性中选择少量种子摩尔数多少 这也是从精英选择中选择的种子摩尔数
    ...
    # Number of mols chosen because of their diversity score 由于多样性得分而选择的摩尔数
    num_seed_diversity = (
...
def make_pass_through_list(vars, smiles_from_previous_gen_list,
    ...
    generation.
    这一功能决定了哪些精英配体将从上一代进化到下一代而不会被改变的分子。
    Inputs:
    :param dict vars: a dictionary of all user variables
    ...
    :param int num_elite_to_advance_from_previous_gen: the number of molecules
        to advance from the last generation without modifications. 从上一代进化而不进行修饰的分子数量。
    :param int generation_num: the interger of the current generation
    ...
    :returns: list list_of_ligands_to_advance: a list of ligands which should
        ...
        the last generation. Returns a printout of why it failed if it fails list list_of_ligands_to_advance：通过上一代的精英主义，应在不进行修改的情况下进入新一代的配体列表。如果失败则返回失败原因的打印输出
    """ # 这将是一个列表的列表。每个子列表都是 [SMILES_string, ID]
    ...
    # Return None and trigger an Error 如果您的上一代净化不足，无法使列表返回“无”并触发“错误”
    if (
            generation_num != 0
    ...
    if generation_num == 0 and vars["filter_source_compounds"] is True:# 第0代，并且过滤
        # Run Filters on ligand list 在配体列表上运行筛选器
    ...
    # If not enough of your previous generation sanitize to make the list
    # Return None and trigger an Error 如果您的上一代净化不足，无法使列表返回None 并触发Error
    if (
            generation_num != 0
    ...
    # list. 保存通过的所有配体的种子列表，该列表将用作种子列表
    save_ligand_list(
        vars["output_directory"],
        generation_num,
        ligands_which_passed_filters,
    ...
    # # 检查ligands_hich_passed_filters是否有对接分数 
    ...
    if generation_num == 0 and has_dock_score is False:
        ...
        # Use the make_seed_list function to select the list to advance. 使用make_seed_list功能选择要前进的列表。
        # This list will be chosen strictly by 此列表将严格由
        ...
        # Take the 1st num_elite_to_advance_from_previous_gen number of
        # molecules from ligands_which_passed_filters 取分子ligands_which_passed_filters的第1个num_elite_to_advance_from_previous_gen
        random.shuffle(ligands_which_passed_filters)
        list_of_ligands_to_advance = []
        ...
        # list will be chosen strictly by 使用make_seed_list功能选择要前进的列表。此列表将严格由
        list_of_ligands_to_advance = make_seed_list(
            vars,
            ligands_which_passed_filters,
            generation_num,
...
def save_generation_smi(output_directory, generation_num,
    ...
    """
    # folder for this new generation 新生成目录
...
def save_ligand_list(output_directory, generation_num,
    ...
    # make a folder for the Seed files 为种子文件创建一个文件夹
    seed_folder_path = new_gen_folder_path + "SeedFolder" + os.sep

    # check if folders exist, if not make them 检查文件夹是否存在，如果不存在则创建它们
    ...
    )#nomenclature_tag：遗传策略标签

    # save to a new output smiles file. ie. save to ranked_smiles_file 保存到新的输出 smiles 文件。比如保存到ranked_smiles_file
