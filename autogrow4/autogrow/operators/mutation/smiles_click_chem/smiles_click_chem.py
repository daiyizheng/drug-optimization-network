...
class SmilesClickChem(object):
    ...
    def __init__(self, rxn_library_variables, list_of_already_made_smiles,
        ...
        初始化SmilesClickChem。这将建立突变分子所需的所有反应和功能字典
        Inputs:
    ...
    def rxn_lib_format_json_dict_of_dict(self, old_dict):
        ...
        json 字典作为 unicode 类型导入。该脚本将所有键和项目转换为字符串，但有一些特定的例外。它需要官能团字典和反应库。
        ...
        list of strings.
        反应库是字典的字典，并且有一些不打算作为字符串的例外。 比如：num_reactants 转换为整数，function_groups 转换为字符串列表。
        The functional_group_dictionary is simply a dictionary with all items
        and keys needing to be strings.
        function_group_dictionary 只是一个字典，其中所有项目和键都需要是字符串。
        Inputs:
    ...
    def retrieve_functional_group_dict(self, rxn_library, function_group_library):
        ...
        possible reactions. 这将检索各个反应所需的所有官能团的字典。该字典将用于识别可能的反应。
        这是检索将在 SmartClickChem 反应中使用的所有官能团的位置。
        ...
        add a Custom set of reactions please add a folder to如果您想添加更多内容，只需添加一组自定义反应，请将文件夹添加到 
        PATH/autogrow/operators/mutation/smiles_click_chem/Reaction_libraries/.
        They should be formatted as a dictionary of dictionary using the same
        format as : 它们应该被格式化为字典的字典，使用与以下相同的格式：
    ...
    def retrieve_complementary_dictionary(self, rxn_library, complementary_mol_dir):
        ...
        基于用户控制的变量，该定义将检索按功能组分为几类的分子字典。当自动增长最初启动时，应在用户参数测试中处理 .smi 文件的排序。
        Inputs:
        :param str rxn_library: A string defining the choice of the reaction
            library. ClickChem uses the set of reactions from Autogrow 3.1.2.
            Custom means you've defined a path to a Custom library in
    ...
    def determine_functional_groups_in_mol(self, mol_deprotanated, mol_reprotanated):
        ...
        该函数将获取一个分子并找出它具有哪些官能团。这将节省选择反应的时间，特别是当反应列表变得更大时。
        Inputs:
        ...
        for key in list(functional_group_dict.keys()):
            substructure = Chem.MolFromSmarts(functional_group_dict[key]) # Smarts子结构
            if mol_reprotanated.HasSubstructMatch(substructure):# 是否有含有子结构
    ...
    def run_smiles_click(self, ligand_smiles_string):
        ...
        这将采用打乱的反应名称列表（self.shuffled_reaction_list）并测试配体以查看它是否能够在反应中使用。
        Inputs: 如果配体无法在反应中使用，那么我们将继续进行列表中的下一个反应。如果都不起作用，我们返回 None。
        ...
        # try sanitizing, which is necessary later 尝试消毒，稍后需要净化
        ...
        # Determine which functional groups are within a ligand 确定配体内有哪些官能团
        ...
        shuffled_reaction_list = self.rand_key_list(
            self.reaction_dict
        )  # Randomize the order of the list of reactions 随机化反应列表的顺序
        ...
        # of reactions 使用 rxns 的随机顺序遍历 rxns 字典中所有可能的 rxns，当 rxn 成功或反应耗尽时循环结束
        while tries < len(shuffled_reaction_list) and is_rxn_complete is False:
            ...
            for i in range(0, len(fun_groups_in_rxn)):
                if fun_groups_in_rxn[i] in list_subs_within_mol:
                    ...
                    # in the reaction. contains_group 现在等于i 的数字 将用于记住稍后反应中分子的位置。
                    break
            ...
            # deprotanated form of the ligand 确定是否使用配体的质子化或去质子化形式进行反应
            ...
            # to run the reaction 如果反应只需要一种反应物，我们将尝试运行该反应
            if a_reaction_dict["num_reactants"] == 1:
                ...
                try:
                    ...
                    # randomly shuffle the lists of products so that we don't 随机调整产品列表，这样我们就不会偏向单一产品类型。
                    # bias a single product type. ie ClickChem Reactions 即 ClickChem 反应 5_Alkyne_and_Azide 产生两种产物：1,5 异构体和 1,4 异构体；
                    # 5_Alkyne_and_Azide produces two products: a 1,5 isomer 这将打乱列表并尝试每个选项。
                ...
                for i in range(0, len(fun_groups_in_rxn)):
                    if i == contains_group:
                        ...
                        # functional group 对于需要从补充字典中获取的反应物。找到反应物的官能团
                        ...
                        for find_mol_tries in range(0, 100):
                            ...
                            # find that group in the complementary dictionary. 在补充词典中找到该组。
                            ...
                            # Smiles String of complementary molecule Smile互补分子串
                    ...
                    # convert to tuple 我们将创建一个分子元组作为 rdkit mol 对象，首先我们生成一个反应物 mol 对象列表，然后转换为元组
                ...
                # in the terminating. 
                # 运行反应：我们使用 try/ except 语句，以防发生错误并且 rdkit 无法完成反应。否则，无法完成反应将导致终止。
                # Try to run reaction
                try:
                    # if reaction works keep it
                    reaction_products_list = [
                ...
                if (
                    ...
                    is_rxn_complete = False
                    for reaction_product in reaction_products_list:
                        # Filter and check the product is valid 过滤并检查产品是否有效
                        reaction_product_smilestring = self.check_if_product_is_good(
                            reaction_product
                        )
                        if reaction_product_smilestring is None:
