...
def run_main_smiles_merge(vars, lig_string_1, lig_string_2):
    ...
    # This will prevent broken rings. 使用下面的 mcs_H 函数进行最常见的子结构搜索。
    ...
    # confirm that this meets the minimum number of matching atoms 确认这满足匹配原子的最小数量
    if mcs_results.numAtoms < vars["min_atom_match_mcs"]:
        return None

    ### Convert mcs_res from into usable and referable forms 将 mcs_res 转换为可用且可引用的形式
    mcs_mol = Chem.MolFromSmarts(mcs_results.smartsString)

    # handle_mcs_align_labeling_and_cyclicbreaks 处理mcs对齐标记和循环泄漏
    ...
    if mol_1 is None or mol_2 is None or mcs_mol is None:
        return None
    # This is a very big step. This is where all of the atoms are fragmented  这是非常大的一步，原子被碎片化以确定 R 基团选项是什么。
    # to determine what are the R-groups options. R-groups are consolidated   每个分子的 R 组分别合并为 B 组，B 组的 2 个字典被合并为一个主 B 组，该主 B 组被馈送到映射类中，
    ...
    #  映射类将随机选择一组不冲突的 B 组，这样，所选的 2 个 B 都不会与相同的锚点有连接（锚点是共同核心中的原子）。
    rs_chosen_smiles = DnR.handle_dicts_and_select_b_groups(mol_1, mol_2, mcs_mol)
    if rs_chosen_smiles is None:
        return None

    # MERGE ALL CHOSEN R-GROUPS WITH THE CORE 将所有选定的 R 组与核心合并
    ...
    # or None if processing fails ligand_new_smiles 如果处理有效则为 SMILES 字符串，如果处理失败则为 None
