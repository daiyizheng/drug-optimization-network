...
def handle_dicts_and_select_b_groups(mol_1, mol_2, mcs_mol):
    ...
    这需要 3 个 rdkit.Chem.rdchem.Mol 对象 1 用于 lig_1、lig_2 和公共核心 (mcs_mol)。
    Inputs: 它创建所有必要的字典、映射并选择将添加以形成最终分子的配体。
    ...
    # the time this function fails so we will filter this 1st 确认 mcs_mol 可以在 mol_1 和 mol_2 中替换，大约有 0.8% 的时间该函数失败，因此我们将过滤第一个
...
def mol_handling_of_fragmenting_labeling_and_indexing(mol, mcs_mol, lig_number):
    ...
    anchors. is how we determine which R-group goes where relative to the MCS.
     配体需要 rdkit mol，mcs_mol 需要 1。它通过替换 MCS 来片段化配体。它确定每个片段中有哪些锚点。 这些片段是我们的 R 组和锚点的分配。是我们如何确定哪个 R 组相对于 MCS 的去向。
    ...
    # set of molecules touching an Isolabeled core 查找函数中的哪个 MCS Atoms Rs 分支，以查找接触 Isolabeled 核心的一组分子的所有邻居
    mcs_touches = get_atoms_touch_mcs(mol)

    # invert dictionary 倒置字典
    lig_r_atoms_touch_mcs = invert_dictionary(mcs_touches)

    # remove the Core atoms from each ligand this gives us the R-groups 从每个配体中除去核心原子，这给了我们 R 基团
    ...
    if replace_core is None:
        # replace_core failed to handle fragments" Replace_core 无法处理碎片
        return None, None, None
    # 单个锚点（同位素标签）现在可能会在 Replace_core_mols 中多次出现，因为它们是碎片状的。如果失败，replace_frag_w_anchor_isolabels 可以返回 None，所以让我们在继续之前检查 None
    ...
    # MAKE NEW MOL FRAGS FROM LABELED replace_core 从标记的replace_core中制作新的MOL碎片
    ...
    B-字典：Ligmerge 将从 2 个单独的配体中随机选择 R-基团附加到共享的公共核心，但因此公共核心中的锚定原子可能有超过 1 个 R-基团附加到其上。
...
def check_replace_mol(mol_1, mol_2, mcs_mol):
    ...
    确认 mcs_mol 可以在 mol_1 和 mol_2 中替换，大约有 0.8% 的时间该函数失败，因此我们将过滤第一个
    Inputs:
...
def r_group_list(mol, core_mol):
    ...
    # core molecule 这将返回针对核心分子的特定化合物的所有摩尔碎片
...
def replace_core_mol_dummy_atoms(mol, mcs, replace_core_mol):
    ...
    the core atoms. example: 此函数将用核心原子的同位素标签替换虚拟原子 (*)。例子：
    ...
    for atom in replace_core_mol.GetAtoms():
        if atom.GetAtomicNum() == 0: # 化学计算中使用的方法，用于获取原子的原子序数。
...
def r_groups_dict(mol_frags, lig_number_for_multiplier):
    ...
    for each Rgroup
   给定一组 mol_frags 和配体_编号（即 mol_1 为 1，mol_2 为 2），这将为所有 Rgroup 和每个 Rgroup 的所有smile创建字典
    Input
...
def get_atoms_touch_mcs(mol):
    ...
    core atoms. 查找一组接触分子的所有邻居的函数。独立标记的核心原子。
