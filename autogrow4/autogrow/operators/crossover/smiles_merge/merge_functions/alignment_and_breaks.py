...
def handle_mcs_align_labeling_and_cyclicbreaks(mol_1, mol_2, mcs_mol):
    ...
    这将需要 2 个对齐的分子并选择一个特定的对齐，检查是否有任何环和循环断裂和碎片，从共同核心中删除导致这些问题的任何原子，
    Inputs: 重新编号 mol_1、mol_2、mcs_mol 中的同位素原子，使其易于处理，并在其中保持一致他们三个。
    ...
    # Set Alignment, Isotope label, and Handle breaks 设置对齐、同位素标签和处理中断
    picked_alignment = pick_mcs_alignment(mol_1, mol_2, mcs_mol)
    if picked_alignment is None:
        return None, None, None

    # Isotope label the MCS core 同位素标记 MCS 核心
    index_tuple = add_mcs_isolabels(mol_1, mol_2, mcs_mol, picked_alignment)

    ## Reorder all atoms within ligands to be consistent 重新排列排列内部的所有原子以保持一致
    ...
    # SYSTEM ERRORS MAY OCCUR HERE WHEN MCS IS BAD 当 MCS 不良时，可能会出现系统错误 检查循环环断裂，如果断裂则从公共核心中移除原子
    ...
    # Check cyclic ring breaks, if break remove atoms from common core 检查循环环断裂，如果断裂则从公共核心中移除原子
    if are_there_breaks is True:
        # Run cyclic ring break again, if there are still breaks create sys
        # error and abort merge 再次运行循环环中断，如果仍然存在中断，则会创建系统错误并中止合并
    ...
    # Check for any additional fragmentation issues 检查是否有任何其他碎片问题
    confirm_no_frag = Chem.GetMolFrags(mcs_mol, asMols=True, sanitizeFrags=False)
    if len(confirm_no_frag) != 1:
        return None, None, None

    # Renumber all atoms within both ligands to be consistent with the core 重新编号两个配体内的所有原子以与核心一致
    ...
    # tuple of range(0, len(mcs_mol.GetNumAtoms())) 向核心原子添加正确的同位素标签。需要 1 个最终伪对齐元组范围(0, len(mcs_mol.GetNumAtoms()))
    final_alignment = (new_index[2], new_index[2], new_index[2])
    # Isotope label the core 同位素标记核心
    index_tuple = add_mcs_isolabels(mol_1, mol_2, mcs_mol, final_alignment)
    # 用 idx + 标记 R 组中的第一个原子（lig1 为 1000，lig 2 为 2000），1000 与 2000 标签将用于稍后解卷积。这些将是分配给 R 基团中第一个原子的 iso 值。
...
def check_cyclic_breaks(alignment_tuple, mol_1, mol_2, core):
    ...
    通过从核心（MCS 子结构）中移除原子来修复循环中断。去除原子通常会导致碎片（即，从甲基中去除碳会留下 3 个碎片氢）。需要去除碎片原子。假设最大的片段是我们不想删除的感兴趣的核心。
    Inputs:
    ...
    if len(mcs_ringbreak_idx) > 0:
        ...
        # ringbreak idx's we will delete those ringbreak atoms from the core. 这里有点复杂，所以这就是想法：使用断环 idx 列表，我们将从核心中删除那些断环原子。
        # This will change the idx's within the core which is why we've 这将更改核心内的 idx，这就是我们在前面的步骤中对核心进行 iso 标记的原因
        ...
        # the idx's in the original core, as determined by the isotope labels. 一旦我们从核心中删除，我们需要确定碎片原子的 idx，但这些将是原始核心中的 idx，由同位素标签确定。
        # ie. idx_of_atom_in_original_core = atom.GetIsotope()-10000 (We keep
        # the largest fragment as the future core). 如：idx_of_atom_in_original_core =atom.GetIsotope()-10000（我们保留最大的片段作为未来的核心）。
        ...
        # 在我们获得了所有原子 idx（在原始核心中）的所有原子 idx 的列表后，这些原子是环断裂的，以及所有将导致循环断裂/碎片的原子，然后我们将立即删除所有这些原子，从而防止 idx 发生变化的问题当我们删除和碎片问题时。
        # Make a copy of the core to test fragments 复制核心以测试片段
        ...
        # the to delete list 检查碎片并将最小的碎片索引添加到要删除列表中
        ...
        # 现在我们已经制作了一个新的核心，idx 不同，因此我们需要重新标记 mol_1 和 mol_2
        # remove the Iso-labels from lig 1 and 2 for anything deleted 对于删除的任何内容，从 lig 1 和 2 中删除 Iso 标签
        ...
        # removed from mcs. 制作一个新的索引系列，用于将 mol_1 和 mol_2 与核心进行比较。这是使用原始索引和从 mcs 中删除的原子来完成的。
        count = 0
        for l1, l2, c1 in zip(
...
def ringbreak_frag_handling(new_core, mcs_ringbreak_idx):
    ...
    这需要 rdkit 摩尔的核心（在去除原子以解决循环断裂之后）。然后它测试并处理断环和碎片。
    ...
    如果核心中的一个原子被删除，如果它有附加的原子，它可能会导致碎片。所以这一步处理这个问题，并将最大的片段作为新的公共核心。
    Inputs:
    ...
    if num_frag_len > 1:
        # determine the largest fragment in the list of frags 确定碎片列表中最大的碎片
        largest_frag, largest_frag_index_num = find_biggest_frag(check_fragmentation)

        # the core is now the largest fragment 核心现在是最大的碎片
        core = check_fragmentation[largest_frag_index_num]

        # make a list without the largest fragment 制作一个没有最大片段的列表
        list_frag_mols = []
        ...
        # get the idx for all atoms in all frags EXCEPT THE LARGEST FRAG. 获取除最大碎片之外的所有碎片中所有原子的 idx。
        # these will be the idx's of the original common core, before deleting
        # things which will be identified using the Isolabels we added before. 这些将是原始公共核心的 idx，然后删除将使用我们之前添加的 Isolabel 进行标识的内容。
        # We will be deleting these atoms shortly 我们很快就会删除这些原子
...
def add_r_atom_isolabels(mol_1, mol_2):
    ...
    lig 2) the 1000 vs 2000 label will be used to deconvelute later. These
    will be the iso values assigned to the 1st atom in an R-group
    用 idx + 标记 R 组中的第一个原子（lig1 为 1000，lig 2 为 2000），1000 与 2000 标签将用于稍后解卷积。
    Inputs: 这些将是分配给 R 基团中第一个原子的 iso 值
    :param rdkit.Chem.rdchem.Mol mol_1: rdkit mol for ligand 1
    :param rdkit.Chem.rdchem.Mol mol_2: rdkit mol for ligand 2
    """
...
def add_mcs_isolabels(mol_1, mol_2, common_core, picked_alignment):
    ...
    这将修改 mol_1、mol_2 和 common_core 中的每个原子，使其具有基于 common_core 原子索引的相同同位素标签。
    Isotope number is set as the index number for the common_core atoms + 10,000
    同位素编号设置为 common_core 原子的索引编号 + 10,000
    Input
    :param rdkit.Chem.rdchem.Mol mol_1: an rdkit molecule
    :param rdkit.Chem.rdchem.Mol mol_2: an rdkit molecule
...
def renumber_to_mcs(mol, tuple_order_list):
    ...
    # in the tuple. 使元组的长度与 mol 的长度相同，开头需要与 MCS 的对齐编号相同，并且元组中不能有冗余。
    ...
    # reorder the Idx's of the mol to the full_tuple_order 将 mol 的 Idx 重新排序为 full_tuple_order
    mol = Chem.RenumberAtoms(mol, full_tuple_order) # 重新编号化学分子中的原子
    return mol
