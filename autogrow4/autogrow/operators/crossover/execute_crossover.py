...
def test_for_mcs(vars, mol_1, mol_2):
    ...
    采用配体和随机选择的分子并寻找最常见的结构。如果没有正在比较的公共结构，rdFMCS.FindMCS(mols) 会标记错误。Try/ except 语句用于在比较 2 个不同的分子时防止程序崩溃。 mols 是要使用 rdFMCS.FindMCS 进行比较的分子列表
...
def find_random_lig2(vars, ligands_list, ligand1_pair):
    ...
    substructure
    从列表中选择一个随机分子并检查它是否可以转换为 rdkit mol 对象，然后测试满足用户指定的最小共享子结构的最常见子结构 (MCS)
    ...
    while count < len(ligands_list) - 1:
        ...
        # substructure 它发生转换，但不是配体 1。现在让我们测试一个公共子结构
...
def make_crossovers(vars, generation_num, number_of_processors,
    ...
    """
    Make crossover compounds in a list to be returned. 在要返回的列表中制作交叉化合物。

    This runs SmileClick and returns a list of new molecules. 这将运行 SmileClick 并返回新分子的列表。
    ...
    while loop_counter < 2000 and len(new_ligands_list) < num_crossovers_to_make:
        ...
        while len(new_ligands_list) < num_crossovers_to_make and len(react_list) > 0:
            ...
            # make a list of tuples for multi-processing Crossover 为多进程交叉元组列表
            job_input = []
            ...
            # Example information: 信息示例：
            # result is a list of lists 结果是一个列表列表
...
def run_smiles_merge_prescreen(vars, ligands_list, ligand1_pair):
    ...
    # check if ligand_1 can be converted to an rdkit mol 检查ligand_1是否可以转换为rdkit mol
    lig_1 = convert_mol_from_smiles(ligand_1_string)
    if lig_1 is False:
        # Ligand1_string failed to be converted to rdkit mol format
        return None

    # GET TWO UNIQUE LIGANDS TO WITH A SHARED SUBSTRUCTURE 通过共享的结构获得两种独特的配体
...
def do_crossovers_smiles_merge(vars, lig1_smile_pair, ligands_list):
    ...
    # which passed the prescreen. 运行配体的 run_smiles_merge_prescreen 。这得到了一个新的a lig2，它通过了预筛选。
