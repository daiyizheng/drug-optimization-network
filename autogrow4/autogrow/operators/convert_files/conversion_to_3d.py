...
def convert_to_3d(vars, smi_file, smile_file_directory):
    ...
    PDB files.
    此函数使用gypsum将SMILES从1D转换为3D。gypsum将.smi文件中的SMILES转换为3D.sdf文件，然后rdkit将sdf转换为PDB文件。
    Inputs:
...
def convert_smi_to_sdfs_with_gypsum(vars, gen_smiles_file, smile_file_directory):
    ...
    the .smi file. 使用Gypsum将SMILES文件转换为一组3d.sdf文件。这是通过为.smi文件中的每个配体创建一组.json文件来运行Gypsum来实现的。
    ...
    # gypsum_submission_files. 制作一个新的文件夹来放置gypsum.smi和json。将文件夹命名为gypsum_submission_files。
    folder_path = "{}gypsum_submission_files{}".format(smile_file_directory, os.sep)
    if os.path.exists(folder_path) is False:
        os.makedirs(folder_path)

    # Make Output for Gypsum folder (where .sdf's go) 为Gypsum文件夹生成输出（.sdf所在的位置）
    ...
    # Make a folder to put the log files into within the 3D_SDFs folder 在3D_SDFs文件夹中创建一个文件夹以将日志文件放入
    ...
    # Make All of the json files to submit to gypsum 制作所有要提交给gypsum的json文件
...
def make_smi_and_gyspum_params(gen_smiles_file, folder_path,
    ...
    为generation_*_to_overt.smi文件中的每个配体制作一个单独的.smi文件和参数字典提交给Gypsum。
    The .smi file for each ligand will be noted within the dictionary as
    "source". 每个配体的.smi文件将在字典中记为“源”。
    ...
    with open(gen_smiles_file) as smiles_file:
        for line in smiles_file:
            ...
            # Make .json file
            gypsum_params = {
                "source": smi_path,# 分子源文件地址
                "output_folder": gypsum_output_folder_path,# 3D分子文件地址
                "num_processors": 1, # 进程数
                "job_manager": "serial", # 工作管理器
                "use_durrant_lab_filters": True,# 过滤器
                "max_variants_per_compound": max_variance, # Gypsum应制备每个配体多少构象异构体的使用变量
                "thoroughness": gypsum_thoroughness, # Gypsum DL将在多大范围内寻找低能量构象异构体。较大的值会增加运行时间，但可以产生更好的结果
                "separate_output_files": True, # 是否分离文件
                "add_pdb_output": False, # 是否添加到PDB文件中
                "add_html_output": False, # 是否添加到html文件中
                "min_ph": min_ph, # Dimorphite-DL考虑的最小pH值
                "max_ph": max_ph,# Dimorphite-DL考虑的最大pH值
                "pka_precision": pka_precision, # 由Dimorphite-DL确定的pH亚结构范围的大小
                "skip_optimize_geometry": False,# 跳过优化几何体
                "skip_alternate_ring_conformations": False, #跳过交替环构象
                "skip_adding_hydrogen": False, # 跳过加氢
                "skip_making_tautomers": False, # 跳过互变异构体
                "skip_enumerate_chiral_mol": False, # 跳过枚举手性分子
                "skip_enumerate_double_bonds": False, # 跳过枚举双键
                "let_tautomers_change_chirality": False, # 让互变异构体改变手性
                "2d_output_only": False, # 只输出2D结构
                "cache_prerun": False,
                "test": False,
            }
...
def convert_sdf_to_pdbs(vars, gen_folder_path, sdfs_folder_path):
    ...
    # Check that there are .sdf files to test. If not raise Exception 检查是否有要测试的 .sdf 文件。如果没有引发异常
    if len(job_inputs) == 0:
