...
def check_sanitization(mol):
    ...
    rather than causing the outer script to fail. 净化分子
...
def handleHs(mol, protanate_step):
    ...
    absense of H's.去除所有非显式的H并将所有隐含的H加回来。
...
给定一个已经净化和去质子化的rdkit.Chem.rdchem.Mol对象，我们将尝试用隐式Hs对Mol进行再质子化。如果失败，它将返回None，而不是导致外部脚本失败。
    Inputs:
    :param rdkit.Chem.rdchem.Mol sanitized_deprotanated_mol: an rdkit molecule already sanitized and deprotanated.
    Returns:
