...
def prepare_molecules(args):
    ...
    一种为对接准备小分子模型的功能。为了工作，它需要在系统上安装python模块rdkit。
    :param args: The arguments, from the commandline.
    :type args: dict
    """

    # Keep track of the tim the program starts. 记下程序开始的时间。
    start_time = datetime.now()

    # A list of command-line parameters that will be ignored if using a json
    # file. 如果使用json文件，将忽略的命令行参数列表。
    json_warning_list = [
        "source",
        "output_folder",
        "num_processors",
    ...
    if "json" in args:
        ...
        # warning necessary. 我们实际上将使用所有命令行参数。无需警告。
...
def execute_gypsum_dl(contnrs, params):
    ...
    # stereoisomeric forms, etc. 准备smiles。脱盐，考虑交替电离、互变异构、立体异构形式等。
    prepare_smiles(contnrs, params)

    # Convert the processed SMILES strings to 3D. 将处理后的 SMILES 字符串转换为 3D。
    prepare_3d(contnrs, params)

    # Add in name and unique id to each molecule. 为每个分子添加名称和唯一 ID。
    add_mol_id_props(contnrs)

    # Output the current SMILES.
    Utils.print_current_smiles(contnrs)

    # Write any mols that fail entirely to a file. 将所有完全失败的摩尔写入文件。
