...
def run_filter_mol(smiles_info, child_dict):
    ...
    # remove charge from mol objects. This affects some properties 去除摩尔物体上的电荷。这会影响某些属性
    # such as: logP, Mol refractivity, and polar surface area 例如：logP、摩尔折射率和极表面积
    # which can impact filters such as Ghose and VandeWaterbeemd 这可能会影响Ghose和VandeWaterpeed等过滤器
    # This is done because logP is traditionally applied to neutral molecules 这样做是因为logP传统上应用于中性分子
