...
def multiprocess_handling(vars):
    ...
    and adds it to the vars dictionary. 此函数处理多处理函数。它建立了一个Paralelizer对象并将其添加到vars字典中。
    Inputs:
    :param dict vars: dict of user variables which will govern how the programs runs
    Returns:
...
def load_in_commandline_parameters(argv):
    ...
    if "json" in argv:
        ...
        check_for_required_inputs(argv)# 检测参数的合理性
    ...
    print(printout)
    sys.stdout.flush()
    # Check all Dependencies are installed 确实是否安装了依赖
    check_dependencies()

    vars = filter_choice_handling(vars) # 过滤器选择
    ...
    # need to also be specified 检查是否自定义对接选项如果是，还需要指定一些内容
    # if not lets flag the error 如果不允许，则标记错误
    if vars["dock_choice"] == "Custom":# 自定义对接选择
        if vars["docking_executable"] is None:# 自定义对接执行器是否有
            raise ValueError(
                "TO USE Custom DOCKING OPTION, MUST SPECIFY THE \
                PATH TO THE docking_executable AND THE DOCKING_CLASS"
            )
        if os.path.exists(vars["docking_executable"]) is False:# 自定义对接执行器文件是否存在
            raise ValueError(
        ...
        if (
                type(vars["custom_docking_script"]) != list# 自定义对接执行器列表
    ...
    if vars["scoring_choice"] == "Custom":# 得分选择自定义
        if (
    ...
    else:  # Using default settings 默认突变设置
    ...
    # Check if the Operating System is Windows, if so turn off Multiprocessing. 检查操作系统是否为Windows，如果是，请关闭多处理器。
    ...
    # convert path names with spaces if this is windows Windows操作系统的更多处理如果这是Windows，则用空格转换路径名
    if os.name == "nt" or os.name == "ce":
        # so it's running under windows. multiprocessing disabled
    ...
    # CHECK THAT NN1/NN2 are using only traditional Vina Docking 检查NN1/NN2是否仅使用传统的Vina对接
...
此函数用于选择用户定义的配体筛选器。
    ...
    if "No_Filters" in list(vars.keys()):
        ...
        chosen_ligand_filters, vars = picked_filters(vars)# 选择过滤器
    vars["chosen_ligand_filters"] = chosen_ligand_filters

    import autogrow.operators.filter.execute_filters as Filter


    # get child filter class object function dictionary 获取子筛选器类对象函数字典
...
def picked_filters(vars):
    ...
    这将使用用户vars并返回一个分子必须通过才能进入下一代的过滤器列表。
    Inputs:
    :param dict vars: Dictionary of User variables
    Returns:
    :returns: list filter_list: a list of the class of filter which will be used
